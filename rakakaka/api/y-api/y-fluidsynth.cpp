...
// desc: init the synth
//-----------------------------------------------------------------------------
bool GeXFluidSynth::init( int srate, int polyphony )
{
    if( m_synth != NULL )
    {
...
// desc: load a font
//-----------------------------------------------------------------------------
bool GeXFluidSynth::load( const char * filename, const char * extension )
{
...
//-----------------------------------------------------------------------------
void GeXFluidSynth::programChange( int channel, int program )
{
...
// desc: control change
//-----------------------------------------------------------------------------
void GeXFluidSynth::controlChange( int channel, int data2, int data3 )
{
    if( m_synth == NULL ) return;
...
//-----------------------------------------------------------------------------
void GeXFluidSynth::noteOn( int channel, float pitch, int velocity )
{
    // sanity check
    if( m_synth == NULL ) return;
    // integer pitch
...
//-----------------------------------------------------------------------------
void GeXFluidSynth::pitchBend( int channel, float pitchDiff )
{
    // sanity check
    if( m_synth == NULL ) return;
    // lock
...
//-----------------------------------------------------------------------------
void GeXFluidSynth::allNotesOff( int channel )
{
